...
На сторінці JSP є два типи **коментарів**: коментарі на стороні сервера та коментарі на стороні клієнта.

- Коментарі на стороні сервера або приховані коментарі видно лише на стороні сервера, оскільки вони видаляються під час компіляції сторінок JSP.

Коментарі на стороні сервера мають форму

```xml
<% - comments -%>
```

- Клієнтські коментарі, які мають відображатися в створеному документі, надісланому клієнту. Вони не повинні містити інформацію про поведінку та внутрішню структуру серверного додатку або код для генерування відповідей.
